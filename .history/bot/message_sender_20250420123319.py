...
class MessageHandler:
    ...
    async def handle_api_request(self, func, *args, **kwargs):
        ...
        for attempt in range(self.retry_count):
            try:
                ...
                result = func(*args, **kwargs)  # Remove await here since func is synchronous
                return result
    ...
    async def send_message(self, user_id, text):
        try:
            result = self.client.direct_send(text=text, user_ids=[user_id])
    ...
    async def get_messages(self, thread_id):
        try:
            return self.client.direct_messages(thread_id)
        except Exception as e:
